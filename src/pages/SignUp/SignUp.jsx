...
import { GoogleIcon, FacebookIcon } from './components/CustomIcons.jsx'; // SitemarkIcon видалено
...
const Card = styled(MuiCard)(({ theme }) => ({
  ...
  alignSelf: 'center',
  width: '100%',
  padding: theme.spacing(3), // Зменшено загальний padding картки
  gap: theme.spacing(1.5),   // Зменшено проміжки між елементами в картці
  margin: 'auto',
  boxShadow: // Залишаємо ваші оригінальні тіні
      'hsla(220, 30%, 5%, 0.05) 0px 5px 15px 0px, hsla(220, 25%, 10%, 0.05) 0px 15px 35px -5px',
  [theme.breakpoints.up('sm')]: {
    width: '450px', // Залишено, можна змінити на maxWidth, якщо потрібно
...
const SignUpContainer = styled(Stack)(({ theme }) => ({
  ...
  // Залишаємо ваші оригінальні фони
  '&::before': {
    content: '""',
    display: 'block',
    position: 'absolute',
...
export default function SignUp(props) {
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      ...
      window.location.href = '/signin'; // Перенаправлення після успіху
    } catch (error) {
  ...
  return (
      <AppTheme {...props}>
        ...
        <SignUpContainer direction="column" justifyContent="center"> {/* Змінено justifyContent */}
          {/* alignItems: 'flex-start' для картки, щоб контент починався зліва */}
          <Card variant="outlined" sx={{ alignItems: 'center' }}>
            {/* <SitemarkIcon /> Замінено на Typography */}
            <Typography
                variant="h5" // Зменшено розмір
                component="div"
                sx={{
                  fontWeight: 700,
                  letterSpacing: '.15rem', // Зменшено letterSpacing
            ...
            <Typography
                component="h1" // Головний заголовок сторінки
                variant="h4"
                sx={{
                  width: '100%',
                  fontSize: 'clamp(1.8rem, 8vw, 2rem)', // Трохи зменшено
                  // textAlign: 'left', // Якщо потрібно
            ...
            <Box
                ...
                noValidate // Додав noValidate, оскільки валідація кастомна
                sx={{
                  display: 'flex',
                  flexDirection: 'column',
                  width: '100%',
            ...
            >
              ...
              <FormControl>
                ...
                <TextField
                    ...
                    color={emailError ? 'error' : 'primary'} // Змінено з passwordError на emailError
                    size="small"
                />
              </FormControl>
              <FormControl>
              ...
              <FormControlLabel
                  control={<Checkbox value="allowExtraEmails" color="primary" size="small"/>} // Зменшено розмір чекбоксу
              ...
              <Button
                  ...
                  fullWidth
                  variant="contained"
                  color="primary" // Використовує колір з теми
                  //   onClick={validateInputs} // Тепер викликається з handleSubmit
                  sx={{ mt: 1, py: 1.2 }} // Зменшив вертикальний padding
              >
                Зареєструватися
              </Button>
            </Box>
            <Divider sx={{width: '100%', my: 1.5}}> {/* Розширив Divider */}
            ...
            <Box sx={{ display: 'flex', flexDirection: 'column', gap: 1.5, width: '100%' }}> {/* width: 100% */}
              <Button
                  ...
                  variant="outlined"
                  color="primary" // Використовує колір з теми
                  onClick={() => alert('Реєстрація через Google')}
              ...
              <Typography sx={{ textAlign: 'center', pt: 1 }}> {/* Додав невеликий відступ */}
